class StatisticsController < ApplicationController 
  ...
  def index
    ...
    @total_expenses            = @cars.total_items(*params_m, items= :expenses)
    ...
    @all_car_expenses          = @cars.all_car_items(*params_m, items= :expenses)
    @total                     = @total_repairs.merge(@total_chargings, @total_taxes, @total_expenses) { |_,a,b| a+b }  
    ...
    @max_length_expenses       = @total_expenses.values.max
    ...
    @total_expenses_year       = @cars.total_items_year(*params_y, items= :expenses)
    ...
    @all_car_expenses_year        = @cars.all_car_items_year(*params_y, items= :expenses)
    @total_year                = @total_repairs_year.merge(@total_chargings_year, @total_taxes_year, @total_expenses_year) { |_,a,b| a+b }
    ...
    @max_length_expenses_year  = @total_expenses_year.values.max
